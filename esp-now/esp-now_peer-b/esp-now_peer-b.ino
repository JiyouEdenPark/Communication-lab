...
static const uint8_t PEER_A_MAC[6] = {0x54, 0x32, 0x04, 0x3E, 0xFC, 0xD8};
...
static void onDataRecv(const esp_now_recv_info *info, const uint8_t *data, int len) {
  ...
  String peerAMac = macToString(PEER_A_MAC);
  ...
  if (senderMac == peerAMac) {
    Serial.println("[INFO] Message from PEER_A");
    // Process commands from PEER_A
    if (s == "ON") {
      setLed(true);
    } else if (s == "OFF") {
      setLed(false);
    ...
    Serial.print("[WARNING] Unknown sender: ");
    Serial.println(senderMac);
    // Optionally process commands from unknown senders
    if (s == "TOGGLE") {
      toggleLed();
    }
  }
...
void setup() {
  ...
  printMac12(selfMac);
  Serial.println();
  Serial.println("ESP-NOW PEER_B ready.");
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\r' || c == '\n') {
      ...
      if (line.length() > 0) {
        ...
        line = "";
        cmd.trim();
        sendToPeer(PEER_A_MAC, cmd.c_str());
      }
    } else {
